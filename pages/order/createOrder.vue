<template>
	<view>
		...
		<view class="goods-section">
			<!-- 商品列表 -->
			<view class="g-item" v-for="(item, index) in cartList" :key="item.id">
				<image :src="getAvatarView(item.viewImage)"></image>
				<view class="right">
					<text class="title clamp">{{item.productName}}</text>
					<text class="spec">{{item.specsName}}</text>
					<view class="price-box">
						<text class="price">¥{{item.price}}</text>
						<text class="number">{{item.buyNum}}</text>
					</view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			async loadAddress(){
				...
				uni.request({
					url: this.$baseUrl + '/usr/address/getAddress',
					data: {
						userId: userId,
					},
					header: {
